  ...
  switch(c->land) {
    ...
    case laBlizzard:
      ...
      if(fargen) {
        ...
        if(windless) {
          c->wall = waIcewall;
          if(hrand(500) < PT(100 + 2 * kills[moVoidBeast] + 2 * kills[moIceGolem] + 2 * kills[moWinterElemental], 200) && notDippingFor(itBlizzard))
            c->item = itBlizzard;
      ...
      if(d == 8) c->landparam = 0;
      ONEMPTY {
        if(hrand_monster(40000) < 10 + (items[itBlizzard] + yendor::hardness()))
          c->monst = moWinterElemental;
	      else if(hrand_monster(8000) < 10 + (items[itBlizzard] + yendor::hardness()))
    ...
    case laHurricane:
      ...
      if(d == 7 && !safety) {
        if(hrand(5000) < 500)
           c->wall = waBoat;
        else if(hrand_monster(200000) < 10 + 2*itThunderStone + yendor::hardness())
          c->monst = moStormElemental;
        else if(hrand_monster(75000) < 10 + 2*itThunderStone + yendor::hardness())
          c->monst = moAlbatross;
        else if(hrand_monster(130000) < 10 + 2*itThunderStone + yendor::hardness()) {
          c->wall = waBoat;
          c->monst = moPirate;
          }
